#!/usr/bin/env python3
"""
F006: Trait-Predictiveness & Hybrid Item Selection Strategy Evaluation.

Runs 5-fold participant-level CV with four psychometric selection strategies
across four item-count ratios (10, 30, 50, 90 % of 100 items).

Strategies evaluated:

- **TraitPredictiveness**: corrected item-total correlation |r_i|, top-m
- **Hybrid-A**: Coverage + TraitPredictiveness (greedy, z-score combined)
- **Hybrid-B**: Coverage + TraitPredictiveness − Redundancy
- **Hybrid-C**: Coverage + TraitPredictiveness − Redundancy
               − TraitImbalancePenalty − DirectionImbalancePenalty

All selection decisions use **train participants only** (AC003).  Weights
are fixed: α=1.0, β=1.0, γ=0.5, δ=0.5.

Prediction: per-subject item-level KNN (K=5, cosine distance on E_old
embeddings).  Evaluation: item-level Pearson r and trait-level per-trait r
via ``cv_framework.evaluate_predictions``.

Usage::

    python scripts/run_trait_hybrid_selection.py              # full run
    python scripts/run_trait_hybrid_selection.py --quick       # 1 fold, 2 ratios
    python scripts/run_trait_hybrid_selection.py --smoke       # 1 fold, 1 ratio
"""

from __future__ import annotations

import sys
import time
import warnings
from pathlib import Path

import numpy as np
import pandas as pd

# Suppress scipy ConstantInputWarning during per-subject correlation computation
warnings.filterwarnings("ignore", category=RuntimeWarning)
warnings.filterwarnings("ignore", message="An input array is constant")

# ---------------------------------------------------------------------------
# Paths & constants
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = PROJECT_ROOT / "data" / "processed"
OUTPUT_DIR = PROJECT_ROOT / "results" / "phase1"

RANDOM_STATE = 0
K_NEIGHBORS = 5
N_FOLDS = 5
RATIOS = (10, 30, 50, 90)
TRAIT_ORDER = ("O", "C", "E", "A", "N")

# Hybrid weights (fixed; could be tuned via inner validation in future work)
ALPHA = 1.0   # TraitPredictiveness
BETA = 1.0    # Redundancy penalty
GAMMA = 0.5   # Trait-imbalance penalty
DELTA = 0.5   # Direction-imbalance penalty

STRATEGY_LABELS = (
    "TraitPredictiveness",
    "Hybrid-A",
    "Hybrid-B",
    "Hybrid-C",
)


def _resolve_imports():
    """Make local scripts importable."""
    scripts_dir = str(PROJECT_ROOT / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)


_resolve_imports()
from cv_framework import (  # noqa: E402
    participant_cv_split,
    evaluate_predictions,
)
from selection import (  # noqa: E402
    TraitPredictivenessSelector,
    HybridSelector,
)


# ---------------------------------------------------------------------------
# Data loading
# ---------------------------------------------------------------------------


def load_data() -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    """Load F001 pre-processed data.

    Returns
    -------
    Y : np.ndarray  (2749, 100) float64 — reverse-scored responses
    E_old : np.ndarray  (100, 1024) float64 — L2-normalised SBERT embeddings
    metadata : pd.DataFrame  (100, 4)
    """
    Y = np.load(DATA_DIR / "Y.npy").astype(np.float64)
    E_old = np.load(DATA_DIR / "E_old.npy").astype(np.float64)
    metadata = pd.read_parquet(DATA_DIR / "metadata.parquet")
    print(f"[OK] Loaded Y:      {Y.shape}  range=[{Y.min():.0f}, {Y.max():.0f}]")
    print(f"[OK] Loaded E_old:  {E_old.shape}")
    print(f"[OK] Loaded metadata: {len(metadata)} items")
    return Y, E_old, metadata


# ---------------------------------------------------------------------------
# Precompute distance matrix & vectorised KNN prediction
# ---------------------------------------------------------------------------


def _precompute_cosine_dist(E: np.ndarray) -> np.ndarray:
    """Compute cosine distance matrix from L2-normalised embeddings.

    For L2-normalised vectors, ``cosine_dist(i,j) = 1 - dot(e_i, e_j)``.
    Returns shape ``(n_items, n_items)``.
    """
    sim = E @ E.T  # (100, 100)
    sim = np.clip(sim, -1.0, 1.0)
    dist = 1.0 - sim
    np.fill_diagonal(dist, np.inf)  # prevent self-match
    return dist


def _predict_held_out_batch(
    y_test: np.ndarray,
    dist: np.ndarray,
    S: np.ndarray,
    k: int = K_NEIGHBORS,
) -> np.ndarray:
    """Predict held-out items for ALL test subjects in one vectorised pass.

    Parameters
    ----------
    y_test : np.ndarray  shape (n_test, n_items)
        Test-subject response matrix (reverse-scored).
    dist : np.ndarray  shape (n_items, n_items)
        Precomputed cosine distance matrix.
    S : np.ndarray  shape (|S|,)
        Indices of selected (observed) items.
    k : int
        Number of neighbours (capped at ``|S|``).

    Returns
    -------
    y_pred : np.ndarray  shape (n_test, n_items)
        Predictions.  Items in *S* are ``NaN``.
    """
    n_items = dist.shape[0]
    T_mask = np.ones(n_items, dtype=bool)
    T_mask[S] = False
    T = np.where(T_mask)[0]

    y_pred = np.full_like(y_test, np.nan)

    if len(T) == 0:
        return y_pred

    k_eff = min(k, len(S))

    # For each held-out item j, find its k nearest neighbours in S
    d_st = dist[np.ix_(S, T)]  # (|S|, |T|)

    if k_eff < len(S):
        nn_idx_in_S = np.argpartition(d_st, k_eff - 1, axis=0)[:k_eff]
    else:
        nn_idx_in_S = np.arange(len(S))[:, None].repeat(len(T), axis=1)

    nn_items = S[nn_idx_in_S]  # (k_eff, |T|)

    for si in range(y_test.shape[0]):
        y_subj = y_test[si]
        preds = y_subj[nn_items].mean(axis=0)
        preds = np.round(preds)
        preds = np.clip(preds, 1, 5)
        y_pred[si, T] = preds

    return y_pred


# ---------------------------------------------------------------------------
# Single fold evaluation
# ---------------------------------------------------------------------------


def _eval_fold(
    y_train: np.ndarray,
    y_test: np.ndarray,
    D: np.ndarray,
    E_old: np.ndarray,
    trait_ids: np.ndarray,
    reverse_ids: np.ndarray,
    fold_idx: int,
    quick: bool = False,
    smoke: bool = False,
) -> list[dict]:
    """Evaluate all 4 strategy × ratio combinations for one outer fold.

    Parameters
    ----------
    y_train : np.ndarray  shape (n_train, 100)
        Train-subject response matrix — used for item selection (AC003).
    y_test : np.ndarray  shape (n_test, 100)
        Test-subject response matrix.
    D : np.ndarray  shape (100, 100)
        Precomputed cosine distance matrix.
    E_old : np.ndarray  shape (100, d)
        L2-normalised SBERT embeddings.
    trait_ids, reverse_ids : np.ndarray
        Item metadata for ``evaluate_predictions``.
    fold_idx : int
        Fold number (for logging).
    quick : bool
        If True, evaluate only ratios 10 and 50.
    smoke : bool
        If True, evaluate only ratio 10.

    Returns
    -------
    rows : list of dict
        One dict per strategy×ratio with metric values and diagnostics.
    """
    ratios = (10,) if smoke else ((10, 50) if quick else RATIOS)
    rows: list[dict] = []

    # Pre-build selectors that depend on y_train (AC003: train only)
    tps = TraitPredictivenessSelector(y_train, trait_ids, TRAIT_ORDER)
    hs_a = HybridSelector(
        E_old, y_train, trait_ids, reverse_ids,
        variant="A", alpha=ALPHA, beta=BETA, gamma=GAMMA, delta=DELTA,
    )
    hs_b = HybridSelector(
        E_old, y_train, trait_ids, reverse_ids,
        variant="B", alpha=ALPHA, beta=BETA, gamma=GAMMA, delta=DELTA,
    )
    hs_c = HybridSelector(
        E_old, y_train, trait_ids, reverse_ids,
        variant="C", alpha=ALPHA, beta=BETA, gamma=GAMMA, delta=DELTA,
    )

    selectors: list[tuple[str, object]] = [
        ("TraitPredictiveness", tps),
        ("Hybrid-A", hs_a),
        ("Hybrid-B", hs_b),
        ("Hybrid-C", hs_c),
    ]

    for m in ratios:
        t0 = time.time()
        best_item_r = -np.inf
        best_label = ""

        for label, selector in selectors:
            S = selector.select(m)
            y_pred = _predict_held_out_batch(y_test, D, S, k=K_NEIGHBORS)
            metrics = evaluate_predictions(y_test, y_pred, trait_ids, reverse_ids)

            # Compute diagnostics
            cov_val = selector.compute_coverage(S) if hasattr(selector, "compute_coverage") else np.nan
            red_val = selector.compute_redundancy(S) if hasattr(selector, "compute_redundancy") else np.nan

            # Trait balance diagnostics
            trait_counts = {t: int((trait_ids[S] == t).sum()) for t in TRAIT_ORDER}
            ideal = m / len(TRAIT_ORDER)
            trait_max_dev = max(abs(c - ideal) for c in trait_counts.values())

            # Direction balance
            fwd_count = int((reverse_ids[S] == 0).sum())
            fwd_ratio = fwd_count / m if m > 0 else 0.5
            dir_imbalance = abs(fwd_ratio - 0.5)

            row = _make_row(
                strategy=label, ratio=m, fold=fold_idx,
                metrics=metrics, selected_S=S,
                coverage_val=cov_val, redundancy_val=red_val,
                trait_max_dev=trait_max_dev, dir_imbalance=dir_imbalance,
                trait_counts=trait_counts, fwd_ratio=fwd_ratio,
            )
            rows.append(row)

            item_r = metrics["item_level"]["pearson_r"][0]
            if item_r > best_item_r:
                best_item_r = item_r
                best_label = label

        elapsed = time.time() - t0
        print(f"  [Fold {fold_idx + 1}] m={m:>2}  "
              f"best: {best_label} (item_r={best_item_r:.4f})  "
              f"({elapsed:.1f}s)")

    return rows


def _make_row(
    strategy: str,
    ratio: int,
    fold: int,
    metrics: dict,
    selected_S: np.ndarray,
    coverage_val: float,
    redundancy_val: float,
    trait_max_dev: float,
    dir_imbalance: float,
    trait_counts: dict,
    fwd_ratio: float,
) -> dict:
    """Pack evaluation results into a flat dictionary row."""
    il = metrics["item_level"]
    tl = metrics["trait_level"]
    pc = metrics["profile_correlation"]

    row = {
        "strategy": strategy,
        "ratio": ratio,
        "fold": fold,
        "item_r": il["pearson_r"][0],
        "item_r_ci_lower": il["pearson_r"][1],
        "item_r_ci_upper": il["pearson_r"][2],
        "item_mae": il["mae"][0],
        "item_rmse": il["rmse"][0],
        "item_rounded_accuracy": il["rounded_accuracy"][0],
        "trait_r_O": tl["per_trait_r"].get("O", np.nan),
        "trait_r_C": tl["per_trait_r"].get("C", np.nan),
        "trait_r_E": tl["per_trait_r"].get("E", np.nan),
        "trait_r_A": tl["per_trait_r"].get("A", np.nan),
        "trait_r_N": tl["per_trait_r"].get("N", np.nan),
        "trait_r_mean": tl["mean_big5_r"],
        "profile_r": pc["mean"],
        "coverage": coverage_val,
        "redundancy": redundancy_val,
        "trait_max_dev": trait_max_dev,
        "dir_imbalance": dir_imbalance,
        "fwd_ratio": fwd_ratio,
        "trait_count_O": trait_counts.get("O", 0),
        "trait_count_C": trait_counts.get("C", 0),
        "trait_count_E": trait_counts.get("E", 0),
        "trait_count_A": trait_counts.get("A", 0),
        "trait_count_N": trait_counts.get("N", 0),
        "selected_S": ",".join(str(i) for i in selected_S),
    }
    return row


# ---------------------------------------------------------------------------
# Aggregate across folds
# ---------------------------------------------------------------------------


def _aggregate_results(detail_rows: list[dict]) -> pd.DataFrame:
    """Compute per-strategy×ratio mean metrics by averaging across folds."""
    df_detail = pd.DataFrame(detail_rows)
    metric_cols = [
        "item_r", "item_mae", "item_rmse", "item_rounded_accuracy",
    ] + [f"trait_r_{t}" for t in TRAIT_ORDER] + [
        "trait_r_mean", "profile_r", "coverage", "redundancy",
        "trait_max_dev", "dir_imbalance", "fwd_ratio",
    ]
    group_cols = ["strategy", "ratio"]
    agg = df_detail.groupby(group_cols)[metric_cols].mean().reset_index()
    return agg


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------


def main() -> int:
    quick = "--quick" in sys.argv
    smoke = "--smoke" in sys.argv

    mode_str = "SMOKE (1 fold, 1 ratio)" if smoke else (
        "QUICK (1 fold, 2 ratios)" if quick else "FULL"
    )
    print("=" * 60)
    print(f"F006: Trait & Hybrid Selection Evaluation — {mode_str}")
    print("=" * 60)

    # 1. Load data
    print("\n[1/5] Loading data ...")
    Y, E_old, metadata = load_data()
    trait_ids = metadata["trait_id"].values
    reverse_ids = metadata["reverse_id"].values.astype(np.float64)
    n_subjects = Y.shape[0]

    # Precompute cosine distance matrix (once for entire run)
    D = _precompute_cosine_dist(E_old)
    print(f"[OK] Cosine distance matrix: {D.shape}")

    # 2. Strategy overview
    print("\n[2/5] Strategy overview ...")
    print(f"    TraitPredictiveness: top-m by |corrected item-total r|")
    print(f"    Hybrid-A: Coverage + TraitPredictiveness (α={ALPHA})")
    print(f"    Hybrid-B: Coverage + TraitPredictiveness − Redundancy "
          f"(α={ALPHA}, β={BETA})")
    print(f"    Hybrid-C: Coverage + TraitPredictiveness − Redundancy "
          f"− ImbalancePenalty (α={ALPHA}, β={BETA}, γ={GAMMA}, δ={DELTA})")
    print(f"    All selection on train participants only (AC003)")

    # 3. Participant CV split
    print(f"\n[3/5] Running {N_FOLDS}-fold participant CV ...")
    folds = participant_cv_split(n_subjects, n_folds=N_FOLDS, seed=RANDOM_STATE)

    actual_folds = 1 if smoke else N_FOLDS
    all_rows: list[dict] = []

    for fold_idx, (train_idx, test_idx) in enumerate(folds[:actual_folds]):
        y_train = Y[train_idx]  # (n_train, 100) — for item selection
        y_test = Y[test_idx]    # (n_test, 100)
        print(f"\n--- Fold {fold_idx + 1}/{actual_folds} "
              f"(train={len(train_idx)}, test={len(test_idx)}) ---")

        fold_rows = _eval_fold(
            y_train=y_train,
            y_test=y_test,
            D=D,
            E_old=E_old,
            trait_ids=trait_ids,
            reverse_ids=reverse_ids,
            fold_idx=fold_idx,
            quick=quick,
            smoke=smoke,
        )
        all_rows.extend(fold_rows)

    # 4. Aggregate
    print("\n[4/5] Aggregating results ...")
    df_detail = pd.DataFrame(all_rows)
    df_agg = _aggregate_results(all_rows)

    # Print summary
    print("\n--- Per-strategy × ratio (averaged across folds) ---")
    header = f"{'Strategy':<24} {'m':>3}  {'item_r':>8}  {'big5_r':>8}  " \
             f"{'coverage':>8}  {'trait_dev':>10}  {'dir_imb':>8}"
    print(header)
    print("-" * len(header))
    for _, row in df_agg.iterrows():
        print(f"{row['strategy']:<24} {int(row['ratio']):>3}  "
              f"{row['item_r']:>8.4f}  {row['trait_r_mean']:>8.4f}  "
              f"{row['coverage']:>8.4f}  {row['trait_max_dev']:>10.2f}  "
              f"{row['dir_imbalance']:>8.4f}")

    # Per-trait breakdown for m=30
    print("\n--- Per-trait r at m=30 ---")
    m30 = df_agg[df_agg["ratio"] == 30]
    for _, row in m30.iterrows():
        traits_str = " ".join(
            f"{t}={row[f'trait_r_{t}']:.4f}" for t in TRAIT_ORDER
        )
        print(f"  {row['strategy']:<24} {traits_str}")

    # 5. Save
    print(f"\n[5/5] Saving results ...")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    detail_path = OUTPUT_DIR / "trait_hybrid_selection_detail.csv"
    df_detail.to_csv(detail_path, index=False)
    print(f"[OK] Detail results ({len(df_detail)} rows) → {detail_path}")

    agg_path = OUTPUT_DIR / "trait_hybrid_selection_aggregated.csv"
    df_agg.to_csv(agg_path, index=False)
    print(f"[OK] Aggregated results ({len(df_agg)} rows) → {agg_path}")

    summary_path = OUTPUT_DIR / "trait_hybrid_selection_summary.csv"
    # Summary: same as aggregated (already averaged across folds)
    df_agg.to_csv(summary_path, index=False)
    print(f"[OK] Summary ({len(df_agg)} rows) → {summary_path}")

    print("\n" + "=" * 60)
    print("F006 COMPLETE.")
    print("=" * 60)
    return 0


if __name__ == "__main__":
    sys.exit(main())
